...
function AssetsModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      <div
        className="rounded-2xl w-full max-w-4xl md:max-w-5xl lg:max-w-6xl h-[80vh] flex flex-col shadow-2xl border bg-white dark:bg-[#1D1D1D] border-gray-200 dark:border-[#2A2A2A]"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-gray-300 dark:border-[#2A2A2A]">
          <h2 className="font-semibold text-sm md:text-lg md:font-bold flex items-center gap-2 text-gray-900 dark:text-[#E8E8E8]">
            <Grid2x2Plus className="w-4 h-4 text-indigo-600 dark:text-[#CEFF00]" />
            Assets Management
          </h2>
          <button
            onClick={onClose}
            className="group/button text-xl w-7 h-7 flex items-center justify-center leading-none transition-all text-gray-500 dark:text-[#7A7A7A] hover:bg-muted hover:text-foreground dark:hover:bg-muted/50 dark:hover:text-[#E8E8E8] rounded-[min(var(--radius-md),12px)] in-data-[slot=button-group]:rounded-lg"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        {/* Code */}
        <div className="flex-1 min-h-0 overflow-hidden p-0 bg-gray-200 dark:bg-[#131313] text-gray-600 dark:text-[#A0A0A0]">
          <AssetsManagement />
        </div>

        {/* Footer */}
        <div className="p-4 border-t m-0! border-gray-300 dark:border-[#2A2A2A] flex gap-3 justify-end">
          <button
            onClick={handleCopy}
            className={cn(
              ...
              copied
                ...
                : "bg-indigo-600 dark:bg-[#CEFF00] text-white dark:text-black hover:bg-indigo-700 dark:hover:bg-[#9cc100]",
            )}
          >
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors
              ...
              hover:border-indigo-500 dark:hover:border-[#CEFF00]/20 hover:bg-indigo-500/20 hover:text-indigo-600 dark:hover:bg-[#9cc100]/10 dark:hover:text-[#CEFF00]"
          >
...
export default function BottomToolbar() {
  ...
  return (
    <>
      ...
      {showClearConfirm && (
        ...
        >
          <div
            ...
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className="font-semibold mb-2 text-gray-900 dark:text-[#E8E8E8]">
              Clear canvas?
            </h3>
            <p className="text-sm mb-5 text-gray-500 dark:text-[#A0A0A0]">
      ...
      <div className="shrink-0 h-10 flex items-center px-3 gap-1 border-t bg-white dark:bg-[#1D1D1D] border-gray-200 dark:border-[#2A2A2A]">
        {/* Tool switcher */}
        <div className="relative flex items-center gap-0 rounded-sm p-0.5 mr-0 bg-gray-100 dark:bg-[#252525]">
          {/* Dynamic Sliding Background */}
          <div
            className="absolute top-0.5 bottom-0.5 left-0.5 w-6 h-6 rounded-sm bg-indigo-600 shadow-sm dark:bg-[#CEFF00] transition-transform duration-200 ease-out-quad"
            style={{
          ...
          {breadcrumb.length > 0 ? (
            breadcrumb.map((node, i) => (
              <span key={node.id} className="flex items-center gap-0.5 min-w-0">
                {i > 0 && (
                  <ChevronRight className="w-3 h-3 shrink-0 text-gray-500 dark:text-[#7A7A7A]" />
                )}
                <button
            ...
            <span className="text-xs text-gray-500 dark:text-[#7A7A7A]">
              No selection
            </span>
          )}
        </div>
              ...
              <TooltipTrigger
                render={
                  <button
                    ...
                    hover:bg-indigo-700 hover:text-white dark:hover:bg-[#D4FF1A] dark:hover:text-black!"
                  >
