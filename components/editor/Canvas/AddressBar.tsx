    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-250! w-full p-0 gap-0 overflow-hidden">
        <DialogHeader className="px-6 pt-6 pb-4">
          <DialogTitle className="text-lg font-semibold text-gray-900 dark:text-[#E8E8E8]">
            Page settings
          </DialogTitle>
          <DialogDescription className="text-sm text-gray-500 dark:text-[#7A7A7A]">
            Edit settings for{" "}
            <span className="font-medium text-indigo-600 dark:text-[#CEFF00]">
              {page?.fileName}
            </span>
          </DialogDescription>
        </DialogHeader>
        ...
        <Tabs defaultValue="meta" className="flex-1 gap-0">
          <div className="px-6 pt-4 border-b border-gray-300 dark:border-[#2A2A2A]">
            <TabsList
              variant="line"
              ...
              {["meta", "open graphs", "styles", "scripts", "favicon"].map(
                (tab) => (
                  <TabsTrigger
                    key={tab}
                    value={tab}
                    className="flex-0 pb-3 px-4 text-sm capitalize rounded-none border-0 data-active:text-indigo-600 dark:data-active:text-[#CEFF00] data-active:after:bg-indigo-600 dark:data-active:after:bg-[#CEFF00] group-data-horizontal/tabs:after:-bottom-0.5!"
                  >
                    {tab === "meta"
                      ? "Meta tags"
          ...
          <div className="px-8 md:px-16 py-8 max-h-125 overflow-y-auto bg-gray-200 dark:bg-[#131313]">
        ...
        <DialogFooter className="px-6 py-4 border-t m-0! border-gray-300 dark:border-[#2A2A2A] bg-gray-50 dark:bg-[#1D1D1D] flex-row justify-end gap-2 rounded-none">
          <Button
            variant="outline"
          ...
          <Button
            onClick={handleSave}
            className="h-10 rounded-sm bg-indigo-600 hover:bg-indigo-700 dark:bg-[#CEFF00] dark:hover:bg-[#D4FF1A] dark:text-black"
          >
            Save changes
          </Button>
        </DialogFooter>
...
const AddressBar = () => {
  ...
  const iconBtnCls =
    "h-6 w-6 flex items-center justify-center rounded transition-colors text-gray-400 dark:text-[#505050] hover:text-indigo-500 dark:hover:text-[#CEFF00] hover:bg-indigo-50 dark:hover:bg-[#CEFF00]/10";

  return (
    <>
      <div className="min-w-0 flex-1 mx-1 md:mx-4">
        <div className="flex flex-1 items-center justify-center">
          <div className="@container group focus-within:border-gray-300 relative bg-white dark:bg-[#212121] flex h-9 flex-1 items-center overflow-hidden rounded-md border border-gray-200 dark:border-[#393939] p-1 transition-all sm:max-w-125">
            <Button
              variant="ghost"
              size="default"
              disabled
            ...
            <div className="shrink-0 rounded-sm w-5 h-5 flex items-center justify-center bg-indigo-500 text-white dark:bg-[#CEFF00] dark:text-black">
              <Feather className="w-3 h-3" />
            </div>

            <div className="flex h-full flex-1 cursor-text items-center pl-2 min-w-0">
              <p className="truncate text-xs font-medium text-slate-700 dark:text-[#7A7A7A] max-w-21 @[280px]:max-w-35 @[380px]:max-w-55 @[460px]:max-w-full">
              ...
              >
                <DropdownMenuTrigger
                  render={
                    <Button
                      variant="ghost"
                      size="default"
                      className="pointer-events-auto h-7 w-7 rounded-md p-1 text-slate-500 hover:bg-indigo-50 hover:text-indigo-700 dark:text-[#7A7A7A] dark:hover:text-[#CEFF00] hidden @[320px]:flex"
                    >
                ...
                <DropdownMenuContent align="end" className="min-w-75 p-1">
                  ...
                  {pageList.map((page) => (
                    <div
                      key={page.id}
                      className="group/item relative flex items-center gap-2 rounded-md px-2 py-2 hover:bg-gray-50 dark:hover:bg-[#2A2A2A] transition-colors"
                      ...
                      {editingPageId === page.id ? (
                        ...
                        >
                          ...
                          <input
                            ...
                            className="flex-1 text-xs border border-indigo-400 dark:border-[#CEFF00] rounded px-2 py-1 bg-white dark:bg-[#1A1A1A] text-gray-800 dark:text-[#E8E8E8] outline-none"
                          />
                          <button
                            onPointerDown={stopProp}
                  ...
                  {isAdding && (
                    ...
                    >
                      <input
                        ...
                        className="flex-1 text-xs rounded-sm border border-gray-200 dark:border-[#383838] bg-white dark:bg-[#1A1A1A] text-gray-800 dark:text-[#E8E8E8] px-2 py-1.5 min-h-8 max-h-8 outline-none focus:border-indigo-400 dark:focus:border-[#CEFF00] placeholder:text-gray-400"
                      />
                      <button
                        onPointerDown={stopProp}
                        onClick={handleAddSave}
                        className="shrink-0 h-8 w-8 flex items-center justify-center rounded-md bg-indigo-600 dark:bg-[#CEFF00] text-white dark:text-black hover:bg-indigo-700 transition-colors"
                      >
                      ...
                      <button
                        ...
                        className="shrink-0 h-8 w-8 flex items-center justify-center rounded-md text-gray-400 hover:bg-gray-100 dark:hover:bg-[#2A2A2A] transition-colors"
                      >
                        <X className="h-4 w-4" />
                      </button>
                    </div>
                  ...
                  <div
                    ...
                    className="flex items-center justify-center gap-2 p-2.25 rounded-md text-sm cursor-pointer text-gray-500 dark:text-[#7A7A7A] hover:text-indigo-500 dark:hover:text-[#CEFF00] hover:bg-indigo-50 dark:hover:bg-[#CEFF00]/10 transition-colors"
                  >
